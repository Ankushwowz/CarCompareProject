<?php
	class Advancedsearch_Model extends CI_Model
	{
		...
		protected $properties_table = 'nof10';
    ...
    public function getrecordCount($jsonPostData) {
		...
		if(!empty($unserialize['miles']) && !empty($unserialize['searchboxForAddress'])){ 
			 $break_array = explode(',', $unserialize['searchboxForAddress']);
			if(!empty($break_array[0])){
					$address = trim($break_array[0]).', '.trim($break_array[1]).', '.trim($break_array[2]).', '.trim($break_array[3]);
					$whererec  = " where short_address ='".$address."' OR long_address='".$address."'";
					$Sql ="select * from ".$this->properties_table.$whererec;
				     $GetResult = $this->db->query($Sql);
					
						//$GetResult = $this->db->where('correct_address',$unserialize['searchboxForAddress'])->get($this->properties_table);
						$resultPremises = $GetResult->row_array();
						if(!empty($resultPremises['lat']) && !empty($resultPremises['lng'])){
							$lat = $resultPremises['lat'];
							$lon = $resultPremises['lng'];
			 ...
			 if(!empty($unserialize['searchboxForAddress'])){
	...
	public function textSearch(){
		...
		if(isset($limitStart ) || !empty($limitStart)) {
			...
			$where  .= " where (short_address like '".$_POST['search']."%' OR long_address like'".$_POST['search']."%')";
			$Sql ="select id,short_address,long_address from  ".$this->properties_table.$where. " LIMIT ".$limitStart." , ".$limitCount;
			...
			if($allcount >0){
				...
				foreach($result as $key=> $row ){
					$response[$key] = $row['short_address'].'-----'.$row['long_address'];
				}
			}else{
	...
	public function textSearchScroll(){
		...
		if(isset($limitStart ) || !empty($limitStart)) {
			if(!empty($_POST['search'])){
			$where  .= " where short_address like '".$_POST['search']."%'";
			$Sql ="select id,short_address from  ".$this->properties_table.$where. " LIMIT ".$limitStart." , ".$limitCount;
			$query = $this->db->query($Sql);
			$allcount = $query->num_rows();
			if($allcount >0){
				$result = $query->result_array();
				foreach($result as $key=> $row ){
					$response[$key] = $row['short_address'];
				}
			}else{
				//$response[0] = 'No Data Found';
			}
	...
	public function saveSearchData($jsonPostData){
		...
		if(!empty($_POST['headersearch'])){
			$rs =  $this->getrecordCountHeader();
			if(!empty($rs['id'])) { $searchid = $rs['id'];}else{ $searchid =0;}
			$totalrows =  $rs['allcount'];
			$search_data = '';
			$single_record_id = $searchid;
			$empty_status = 1;
		}else{
	...
	public function square_lot(){
		$array = array();
		if(!empty($_GET['q'])){
		$where  = " square_lot like '".$_GET['q']."%'";
		$this->db->where($where);
		$query = $this->db->get('square_lot');
		$numRow = $query->num_rows();
		$result = $query->result_array();
		if($numRow >0){
		foreach($result as $key=>$row){
			$array['items'][$key] = $row;
			$array['total_count'] = $numRow;
		}
		}else{
			$array['items'][0]['square_lot'] = 'No record Found';
		}
		return  $array;
		//print_r($array);
		//die();
	...
	public function updateUserSeacrhCartData(){
		if($this->session->userdata('searchid')){
			$browser_id  = session_id();
			$SeacrhSql ="update tbl_save_search set user_id= '".$this->session->userdata('user_id')."' where browser_id= '".$browser_id."'";
		    $this->db->query($SeacrhSql);
			
			$cartSql ="update cart set user_id= '".$this->session->userdata('user_id')."' where browser_id= '".$browser_id."'";
		     return $query =   $this->db->query($cartSql);
		}
		
	}
	/*public function mapdraw($jsonPostData){
		$unserialize = unserialize($jsonPostData);
		$break_array = explode(',', $unserialize['searchboxForAddress']);
			if(!empty($break_array[0])){
				   $GetResult = $this->db->where('premises',$break_array[0])->get($this->properties_table);
					$resultPremises = $GetResult->row_array();
		...
		// Select total records
    public function getrecordCountHeader() {
		$array = array();
	 ...
	 public function getrecordByID($id) {
		  if(!empty($id)){
			  $array =  array();
				...
				$whererec = " where id ='".$id."'";
				$Sql ="select * from ".$this->properties_table.$whererec;
				$query = $this->db->query($Sql);
				$array['allcount'] = $query->num_rows();
				$array['result'] = $query->row_array();
				 return $array;
				}else{ 
				$array['allcount']=0;
				 $array['result'] = '';
